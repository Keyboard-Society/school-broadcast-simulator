...
const Dashboard: React.FC<DashboardProps> = (p) => {
  ...
  return (
    <div className={`dashboard-container ${p.headerMode === "vertical" ? "dashboard-with-vertical-sidebar" : ""}`}>
      <SoundPlayer ref={p.soundPlayerRef} audioSrc="default.mp3" playCount={1} />

      <Row gutter={[20, 20]}>
        {/* ─ 第一行左: 时间+倒计时+开始 ─ */}
        <Col xs={24} sm={12} id="section-time">
        ...
        {/* ─ 第二行左: 番茄钟控件 ─ */}
        <Col xs={24} sm={12} id="section-pomodoro">
        ...
        <Col xs={24} sm={12}>
          <Card style={CARD} bodyStyle={{ padding: 24 }}
            ...
            <PomodoroRings data={pom.data} onConfirm={pom.confirm} />
          </Card>
        </Col>

        {/* ─ 第三行: 时间线 ─ */}
        <Col xs={24} id="section-timeline">
